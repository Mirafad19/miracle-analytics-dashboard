...
export const SelectTrigger = ({ className, children, ...props }: React.ButtonHTMLAttributes<HTMLButtonElement>) => {
  ...
  return (
    <button ref={context.triggerRef} onClick={() => context.setIsOpen(!context.isOpen)} className={`flex items-center justify-between w-full px-3 py-2 text-sm bg-gray-100 dark:bg-zinc-950 border border-zinc-300 dark:border-zinc-800 text-black dark:text-white rounded-md hover:bg-zinc-200 dark:hover:bg-zinc-800 transition-colors duration-200 ${className}`} {...props}>
      {children}
...
export const SelectContent = ({ children, className, ...props }: React.HTMLAttributes<HTMLDivElement>) => {
  ...
  return (
    <div className={`absolute z-50 mt-1 w-full min-w-full left-0 bg-white dark:bg-black backdrop-blur-lg border border-zinc-200 dark:border-zinc-800 rounded-md shadow-lg p-1 ${className}`} {...props}>
      {children}
    </div>
  );
};
...
export const SelectItem = ({ children, value, className, ...props }: React.HTMLAttributes<HTMLDivElement> & { value: string }) => {
  ...
  return (
    <div onClick={handleSelect} className={`px-3 py-2 text-sm text-black dark:text-white rounded-md cursor-pointer hover:bg-zinc-100 dark:hover:bg-zinc-900 ${isSelected ? 'bg-blue-500/20 dark:bg-blue-500/30' : ''} ${className}`} {...props}>
      {children}
    </div>
  );
};