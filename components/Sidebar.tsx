...
const AnalysisBlock = ({
  ...
  const cardClasses = "bg-[#252849]/70 border border-slate-700/50";
    ...
    <div className="space-y-6">
      ...
      {activeSection === 'payment' && (
        <Card className={cardClasses}>
          <CardHeader><CardTitle className="flex items-center gap-2 text-white"><PieChartIcon className="h-5 w-5 text-green-400"/>Revenue by Payment Method</CardTitle></CardHeader>
      ...
      {activeSection === 'expense' && (
        <Card className={cardClasses}>
          <CardHeader><CardTitle className="flex items-center gap-2 text-white"><BarChart3 className="h-5 w-5 text-red-400"/>Expense Distribution</CardTitle></CardHeader>
...
export const Sidebar = ({ 
  ...
  return (
    <div className="fixed inset-0 z-50 flex animate-fade-in">
      ...
      <div className="relative w-full bg-[#1A1D31] shadow-2xl overflow-y-auto p-6 space-y-6">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3"><div className="p-2 bg-slate-900/50 rounded-lg border border-slate-700"><BarChart3 className="h-6 w-6 text-purple-400" /></div><h2 className="text-2xl font-bold text-white">Detailed Analytics</h2></div>
          <Button onClick={onClose} className="text-white hover:bg-white/10 rounded-full p-2 h-auto"><X className="h-5 w-5" /></Button>
        </div>

        <div className="flex gap-4">
          <Button onClick={() => handleSectionChange('payment')} className={`${activeSection === 'payment' ? 'bg-gradient-to-r from-green-500 to-emerald-600 text-white' : 'bg-[#252849]/70 border border-slate-700/50 text-white hover:bg-slate-800/60'} transition-all`}><PieChartIcon className="h-4 w-4 mr-2" />Payment Mode Analysis</Button>
          <Button onClick={() => handleSectionChange('expense')} className={`${activeSection === 'expense' ? 'bg-gradient-to-r from-red-500 to-pink-600 text-white' : 'bg-[#252849]/70 border border-slate-700/50 text-white hover:bg-slate-800/60'} transition-all`}><TrendingUp className="h-4 w-4 mr-2" />Expenditures Analysis</Button>
        </div>
                ...
                <CardContent>
                    <div className="max-h-96 overflow-y-auto">
                        <table className="w-full text-sm">
                            {activeSection === 'payment' ? (
                                <>
                                    <thead><tr className="sticky top-0 bg-slate-800/80 backdrop-blur-sm"><th className="p-3 text-left text-slate-300">Date</th><th className="p-3 text-left text-slate-300">Patient Name</th><th className="p-3 text-left text-slate-300">Purpose</th><th className="p-3 text-right text-slate-300">Amount</th></tr></thead>
                                    <tbody>{selectedSegmentData.map((record, index) => isFinancialRecord(record) && (<tr key={index} className="border-b border-slate-700/50 hover:bg-slate-800/40 even:bg-slate-800/20"><td className="p-3 text-blue-300">{formatDate(record.Date)}</td><td className="p-3 text-white">{record.Name || 'N/A'}</td><td className="p-3 text-slate-300">{record.Purpose || 'N/A'}</td><td className="p-3 text-right text-green-400 font-medium">₦{record['Amt. Paid'].toLocaleString('en-NG')}</td></tr>))}</tbody>
                                </>
                            ) : (
                                <>
                                    <thead><tr className="sticky top-0 bg-slate-800/80 backdrop-blur-sm"><th className="p-3 text-left text-slate-300">Date</th><th className="p-3 text-left text-slate-300">Duty</th><th className="p-3 text-left text-slate-300">Purpose</th><th className="p-3 text-right text-slate-300">Amount</th></tr></thead>
                                    <tbody>{selectedSegmentData.map((record, index) => isExpenseRecord(record) && (<tr key={index} className="border-b border-slate-700/50 hover:bg-slate-800/40 even:bg-slate-800/20"><td className="p-3 text-blue-300">{formatDate(record.Date)}</td><td className="p-3 text-white">{record.Duty || 'N/A'}</td><td className="p-3 text-slate-300">{record.Purpose || 'N/A'}</td><td className="p-3 text-right text-red-400 font-medium">₦{record.ExpenseAmount.toLocaleString('en-NG')}</td></tr>))}</tbody>
                                </>
                            )}
                        </table>
                    </div>
        ...
        <Card className={cardClasses}>
            <CardHeader><CardTitle className="flex items-center gap-2 text-white"><FileText className="h-5 w-5 text-purple-400"/>Quick Statistics</CardTitle></CardHeader>
            <CardContent>
                <div className={isCompareMode ? "grid grid-cols-2 gap-4" : "grid grid-cols-2 gap-4"}>
                    <div className={isCompareMode ? "text-center border-r border-slate-700 pr-4" : "text-center"}>
                        {isCompareMode && <h4 className="text-lg font-semibold text-white mb-2">{primaryMonthLabel}</h4>}
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                          <div className="text-center"><div className="text-2xl font-bold text-green-400">₦{totalPrimaryRevenue.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div><div className="text-sm text-blue-200">Total Revenue</div></div>
                          <div className="text-center"><div className="text-2xl font-bold text-red-400">₦{totalPrimaryExpenses.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div><div className="text-sm text-blue-200">Total Expenses</div></div>
                        </div>
                    </div>
                    {isCompareMode && (
                        <div className="text-center">
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                              <div className="text-center"><div className="text-2xl font-bold text-green-400">₦{totalCompareRevenue.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div><div className="text-sm text-blue-200">Total Revenue</div></div>
                              <div className="text-center"><div className="text-2xl font-bold text-red-400">₦{totalCompareExpenses.toLocaleString('en-NG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })}</div><div className="text-sm text-blue-200">Total Expenses</div></div>
                            </div>
                        </div>
                    )}
                </div>
