...
import { Sidebar as DetailsSidebar } from './components/Sidebar';
// New robust AppSidebar import
import { AppSidebar } from './components/AppSidebar';
import { Activity, TrendingUp, BarChart3, PieChart as PieChartIcon, Calendar, Filter, Sun, Moon, DollarSign, LogOut } from './components/Icons';
...
export default function Dashboard() {
  ...
  // State for Details Sidebar (Right side)
  ...
  const handleNewFileUpload = (event: ChangeEvent<HTMLInputElement>) => {
      ...
      reader.onload = (e) => {
          try {
              ...
              if (Object.keys(allMonthsData).length === 0) throw new Error('No valid data sheets found.');
  ...
  const handleMonthChange = (month: string) => {
    ...
    if (month === selectedCompareMonth) setSelectedCompareMonth(null);
  };

  const { currentRawData, currentExpenseData } = useMemo(() => {
    if (!monthlyData || !selectedMonth) return { currentRawData: [], currentExpenseData: [] };
    const monthData = monthlyData[selectedMonth];
    return { currentRawData: monthData?.data || [], currentExpenseData: monthData?.expenseCategories || [] };
  }, [monthlyData, selectedMonth]);
  
  const { comparisonRawData, comparisonExpenseCategories } = useMemo(() => {
    if (!monthlyData || !selectedCompareMonth) return { comparisonRawData: [], comparisonExpenseCategories: [] };
    const monthData = monthlyData[selectedCompareMonth];
    return { comparisonRawData: monthData?.data || [], comparisonExpenseCategories: monthData?.expenseCategories || [] };
  ...
  const filteredData = useMemo(() => {
    ...
    if (selectedDuty) dataToFilter = dataToFilter.filter(record => record.Duty === selectedDuty);
  ...
  const calculateKpis = (data: FinancialRecord[]) => {
    const income = data.reduce((sum, record) => {
      if (record['Mode']?.toLowerCase().includes('cash b/f') || record['Mode']?.toLowerCase().includes('cash balance') || record['Mode'] === 'EXP') return sum;
  ...
  const calculateTrendData = (data: FinancialRecord[]) => {
    ...
    data.forEach(record => {
      ...
      if (!dailyData[normalizedDate]) dailyData[normalizedDate] = { income: 0, expenses: 0 };
      if (!record['Mode']?.toLowerCase().includes('cash b/f') && !record['Mode']?.toLowerCase().includes('cash balance') && record['Mode'] !== 'EXP') {
    ...
    return Object.entries(dailyData).map(([date, data]) => ({ day: date.split('-')[0], income: data.income, expenses: data.expenses, netProfit: data.income - data.expenses, }));
  };
  ...
  const mergedTrendData = useMemo<MergedTrendData[]>(() => {
    ...
    if (!selectedCompareMonth) {
      return Array.from(dataMap.values()).map(d => ({ ...d, compareIncome: null, compareExpenses: null, compareNetProfit: null, })).sort((a,b) => Number(a.day) - Number(b.day));
    }
    ...
    return allDays.map(day => {
        ...
        return { day, income: current?.income ?? null, expenses: current?.expenses ?? null, netProfit: current?.netProfit ?? null, compareIncome: compare?.income ?? null, compareExpenses: compare?.expenses ?? null, compareNetProfit: compare?.netProfit ?? null, };
    });
  ...
  const calculateIncomeByPayment = (data: FinancialRecord[]) => {
    ...
    data.forEach(record => {
      ...
      if (mode?.toLowerCase().includes('cash b/f') || mode?.toLowerCase().includes('cash balance') || mode === 'EXP' || amount <= 0) return;
      const normalizedMode = normalizePaymentMethod(mode);
      paymentData[normalizedMode] = (paymentData[normalizedMode] || 0) + amount;
    });
    return Object.entries(paymentData).map(([name, value]) => ({ name, value })).filter(item => item.value > 0).sort((a, b) => b.value - a.value);
  }
  ...
  const calculateExpensesByCategory = (expenseData: ExpenseCategoryRecord[], duty: string | null) => {
    ...
    expenseData.forEach(record => {
      if (duty && record.DUTY && record.DUTY.trim().toLowerCase() !== duty.trim().toLowerCase()) return;
      for (const key in record) {
        if (Object.prototype.hasOwnProperty.call(record, key)) {
          ...
          if (!excludedColumns.includes(trimmedKey.toLowerCase())) {
            ...
            if (value > 0) {
              if (!categoryTotals[trimmedKey]) categoryTotals[trimmedKey] = 0;
    ...
    return Object.entries(categoryTotals).map(([name, value]) => ({ name, value })).filter(item => item.value > 0).sort((a, b) => b.value - a.value);
  }
  ...
  const accountsReceivable = useMemo(() => {
    return filteredData.filter(record => record['To Balance'] > 0).map(record => ({ name: record.Name || 'Unknown Patient', service: record.Purpose || 'Service Not Specified', amount: record['To Balance'], date: record.Date, })).sort((a, b) => b.amount - a.amount);
  }, [filteredData]);

  const comparisonAccountsReceivable = useMemo(() => {
    if (!selectedCompareMonth) return [];
    return processedComparisonData.filter(record => record['To Balance'] > 0).map(record => ({ name: record.Name || 'Unknown Patient', service: record.Purpose || 'Service Not Specified', amount: record['To Balance'], date: record.Date, })).sort((a, b) => b.amount - a.amount);
  ...
  const financialSummary = useMemo(() => {
    const generateSingleMonthSummary = (monthLabel: string, kpisData: any, incomeData: any[], expenseData: any[], arData: any[], totalAr: number) => {
        ...
        if (selectedDuty) summary += `Data is filtered for the ${selectedDuty} department.\n\n`;
    ...
    if (selectedCompareMonth && comparisonKpis) {
        ...
        const calculateChange = (current: number, previous: number) => { if (Math.abs(previous) < 1e-6) return current > 0 ? " (significant increase from zero)" : ""; const change = ((current - previous) / Math.abs(previous)) * 100; return ` (${change >= 0 ? '+' : ''}${change.toFixed(1)}%)`; };
  ...
  }, [kpis, incomeByPayment, expensesByCategory, accountsReceivable, totalReceivables, selectedDuty, selectedMonth, selectedCompareMonth, comparisonKpis, comparisonIncomeByPayment, comparisonExpensesByCategory, comparisonAccountsReceivable, formatCurrency]);
  ...
  const handleSegmentClick = (type: 'payment' | 'expense', segment: string | null, monthContext: 'primary' | 'compare') => {
    ...
    if (type === 'expense' && segment) {
      ...
      expenseCategoriesToFilter.forEach(record => {
        ...
        let expenseAmount = 0;
        if (segment === 'Miscellaneous Expense') { Object.keys(record).forEach(key => { const lowerKey = key.toLowerCase(); if (lowerKey.includes('miscellaneous') || lowerKey.includes('misc')) { expenseAmount += Number(record[key]) || 0; } }); } else { expenseAmount = Number(record[segment]) || 0; }
        if (expenseAmount > 0) { matchingRecords.push({ Date: record.Date || '', Purpose: record.Purpose || 'Expense Transaction', Category: segment, ExpenseAmount: expenseAmount, 'Mode': 'EXP', Duty: record.DUTY || '', Names: '' }); }
      });
      setSelectedSegmentData(matchingRecords);
    } else if (type === 'payment' && segment) {
      const normalizedSegment = normalizePaymentMethod(segment);
      const matchingRecords = dataToFilter.filter(record => { const mode = record['Mode']; const amount = record['Amt. Paid']; if (mode?.toLowerCase().includes('cash b/f') || mode?.toLowerCase().includes('cash balance') || mode === 'EXP' || amount <= 0) return false; return normalizePaymentMethod(mode) === normalizedSegment; });
      setSelectedSegmentData(matchingRecords);
    } else { setSelectedSegmentData([]); }
  };
    ...
    return (
      <div className="min-h-screen bg-black text-white flex flex-col items-center justify-center p-4 relative">
        <div className="absolute top-6 right-6">
          <Button onClick={handleSignOut} className="bg-white/10 backdrop-blur-sm border border-white/20 text-white hover:bg-white/20 transition-colors flex items-center gap-2">
            <LogOut className="h-4 w-4" /> Sign Out
          </Button>
        </div>
        <div className="text-center mb-8">
            <div className="inline-block bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 p-3 rounded-2xl mb-4"><Activity className="h-10 w-10 text-white" /></div>
            <h1 className="text-4xl font-extrabold bg-gradient-to-r from-blue-400 via-purple-500 to-pink-500 bg-clip-text text-transparent mb-2">Miracle Analytics</h1>
  ...
  return (
    <div className={cn(
        "flex flex-col md:flex-row bg-zinc-50 dark:bg-zinc-950 w-full flex-1 mx-auto border-neutral-200 dark:border-neutral-700 overflow-hidden h-screen",
      )}>
        
        {/* Replaced with the new robust AppSidebar */}
        <AppSidebar 
          onUploadClick={() => fileInputRef.current?.click()}
          onProfileClick={() => setIsCreatorModalOpen(true)}
          onSignOutClick={handleSignOut}
        />
        
        <div className="flex-1 flex flex-col overflow-y-auto overflow-x-hidden h-full p-4 sm:p-6 lg:p-8 scroll-smooth">
      ...
      <DetailsSidebar 
          ...
          onClose={() => { setDetailsSidebarOpen(false); setSelectedSegment(null); setDetailsContext(null); }}
